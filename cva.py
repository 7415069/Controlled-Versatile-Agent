...
模型示例（--model 参数，litellm 格式）:
  ...
  groq/llama3-70b-8192          Groq
    ...
    parser.add_argument("--manifest", required=True,
                        help="Role Manifest YAML 文件路径")
    parser.add_argument("--model", default="claude-opus-4-5",
                        help="LiteLLM 格式模型名（默认: claude-opus-4-5）")
    parser.add_argument("--session", default=None,
                        help="恢复指定 session ID 的历史对话")
    parser.add_argument("--list-sessions", action="store_true",
                        help="列出该角色的所有历史 sessions 后退出")
    parser.add_argument("--memory-dir", default="./memory",
                        help="记忆存储目录（默认: ./memory）")
    parser.add_argument("--log-dir", default="./audit-logs",
                        help="审计日志目录（默认: ./audit-logs）")
    parser.add_argument("--max-iterations", type=int, default=100,
                        help="最大推理轮次（默认: 100）")
    parser.add_argument("--max-memory-messages", type=int, default=200,
                        help="内存中最大消息条数（默认: 200）")
    parser.add_argument("--max-token-budget", type=int, default=80000,
                        help="触发上下文截断的 token 估算上限（默认: 80000）")
